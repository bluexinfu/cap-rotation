...
本專案已於 2026-06-16 在 `tide-deploy/` 初始化為 git repo，首個 commit 對應 v0.2.0、並已打 tag `v0.2.0`。CHANGELOG 仍是「給人看」的權威紀錄，git log 是「給機器/細節」的補充，兩者不衝突。

日常使用約定：

1. **commit 訊息用慣例前綴**，與 CHANGELOG 類別對齊：
   - `feat:` 新功能（→ Added）
   - `fix:` 修正（→ Fixed）
   - `refactor:`／`style:` 重構/樣式（→ Changed）
   - `docs:` 文件（→ Docs）
   - `data:` 資料更新（GitHub Actions 已在用這個前綴）
2. **發版時打 tag**：`git tag -a vX.Y.Z -m "..."`，接 GitHub 後 `git push --tags`。
3. 尚未接遠端。要部署到 GitHub Pages 時（步驟見 [README](../README.md)），新增 remote、push，並記得開啟 Actions 寫入權限讓每日自動更新能 commit 回 `data.json`。

> 接上 GitHub 後，可考慮用 GitHub Releases 對應 CHANGELOG 的版本區塊。
> 注意：自動更新的 workflow（`data:` 前綴）會頻繁 commit `data.json`，屬正常資料快照，不需逐筆記入 CHANGELOG。
